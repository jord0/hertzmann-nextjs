...
### Catalog PDFs (Cloudflare R2)

Catalog PDFs are stored in a separate R2 bucket from photos.

- PDF URL helper: `lib/catalog-url.ts` — `catalogPdfUrl(catalogId)` using `NEXT_PUBLIC_CATALOGS_BASE_URL` env var
- Files stored in R2 as `{id}.pdf` (catalog row ID)
- Admin upload API route: `app/api/admin/upload-catalog/route.ts` (to be built)
- Public page: `app/catalogs/page.tsx` — Server Component, queries DB ordered by `level`
...
| `title` | varchar | |
| `date` | varchar(10) | |
| `price` | smallint unsigned | 0 = contact for price |
| `description` | text | |
| `enabled` | tinyint unsigned | 1 = visible on site |
| `level` | smallint | display order (ASC) |

**Required env vars:**
```
NEXT_PUBLIC_CATALOGS_BASE_URL=  # Public R2 URL for catalog bucket, e.g. https://pub-xxxx.r2.dev (no trailing slash)
R2_CATALOG_BUCKET_NAME=         # R2 bucket name for catalog PDFs
```

**Local development:** omit `NEXT_PUBLIC_CATALOGS_BASE_URL` to fall back to `/catalogs`.
