...
async function deletePhotographer(id: number) {
  ...
  // Fetch all photos for this photographer so we can delete from R2
  const photos = (await query(
    'SELECT id FROM photos WHERE photographer = ?',
    [id]
  )) as { id: number }[];

  // Delete each photo from R2
  await Promise.allSettled(photos.map(ph => deletePhoto(id, ph.id)));

  // Delete photos from DB, then photographer
  await query('DELETE FROM photos WHERE photographer = ?', [id]);
...
export default async function EditPhotographerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
        ...
        <div style={{ width: '480px', flexShrink: 0 }}>
          ...
          <form action={updateAction}>
            <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem' }}>
              <Field label="First Name" name="firstName" defaultValue={p.firstName} noMargin />
      ...
      <DeletePhotographerSection action={deleteAction} photoCount={photos.length} />
    </div>
  );
}
